...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Future< void> initializeCamera() async{
    ...
    cameraController = CameraController(frontCamera, ResolutionPreset.ultraHigh,enableAudio: false);
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
             if(isInitialized && _imageFile == null) AspectRatio(aspectRatio:3/4,child: CameraPreview(cameraController!)),
             const SizedBox(height: 10,),
             if(_imageFile != null)  Padding(
               padding:  EdgeInsets.only(top: MediaQuery.of(context).size.height * 0.2),
               child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [ Center(child: Image(width: 180, image: AssetImage('assets/images/face-id.png')),), SizedBox(height: 25,) , Center(child: CupertinoActivityIndicator(radius: 20,color: Colors.green,)), Text('Verifying Face',style: TextStyle(fontWeight: FontWeight.w500,fontSize: 20),), Text('Please wait a few seconds') ],),
