...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> login() async{
    ...
    try{
      if(cnicController.value.text != '') {
        if(cnicController.value.text.length <= 14) {
          final response = await http.post(
              Uri.parse('http://192.168.100.26:5000/login'), headers: {
            'Content-Type': 'application/json',
          }, body: jsonEncode({
            'cnic': cnicController.value.text.trim(),
          }));
          ...
          if (response.statusCode == 200) {
            ...
            debugPrint(
                'Login Failed, status code: ${response.statusCode.toString()}');
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('CNIC must contain 13 digits'),
            backgroundColor: Colors.red,));
        }
      }
      else{
        debugPrint('Please enter correct CNIC');
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Please enter correct CNIC'), backgroundColor: Colors.red,));
      }
  ...
  String? loginValidation(value){
    ...
    debugPrint(cnicController.text.length.toString());

    if(value.isEmpty){
      return "Please enter CNIC eg. 3520237057009";
    }
    if(value.contains('-')){
      return "CNIC must be without dashes";
    }
    if(value.length != 13){
      return "CNIC must consist of 13 digits";
    }
    return null;
  }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Form(autovalidateMode: AutovalidateMode.onUserInteraction,key: formKey, child: Padding(padding: const EdgeInsets.all(0.0), child: InputField(controller: cnicController, hintText: AppLocalizations.of(context)!.enterCNIC, icon: Icons.account_circle,validator: loginValidation,))),
                          ...
                          Padding(
                            ...
                            child: Button(tap: ()async{
                              ...
                              if(widget.auth){
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Verify face inorder to login"), backgroundColor: Colors.red,));
                              }
                              setState(() {
